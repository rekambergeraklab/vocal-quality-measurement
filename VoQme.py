...
class AudioProcessor(QThread):
    ...
    def run(self):
        ...
        try:
            with sd.InputStream(device=self.device_id, channels=1, samplerate=FS, blocksize=BLOCK_SIZE, callback=callback):
...
class VocalMasterGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Vocal Quality Measurement - RBLAB v1.8.0 Deep Diagnosis")
    ...
    def init_ui(self):
        ...
        config_lyt.addWidget(QLabel("INPUT DEVICE", styleSheet=f"color:{TEXT_MUTED}; font-weight:bold; font-size:9pt;"), alignment=Qt.AlignCenter)
        self.device_box = QComboBox(); self.device_box.setStyleSheet(combo_style)
        for i, d in enumerate(sd.query_devices()):
            if d['max_input_channels'] > 0: self.device_box.addItem(d['name'], i)
    ...
    def change_device(self, index):
        ...
        self.processor.device_id = self.device_box.itemData(index)
        self.processor.start()
